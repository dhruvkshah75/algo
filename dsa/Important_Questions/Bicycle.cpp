...
void solve(int n, int m, vector<tuple<int,int,int>> &edges, vector<int> &slow) {
    ...
    // ============= Dijikstra algorithm ==============================
    //dist[cityNode][bikeTaken], is the minimum time to reach city u while riding the bike from city i.
    // make it 2d state so we also calculate for all cases for different bikes taken
